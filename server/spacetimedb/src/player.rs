...
pub fn join_game(ctx: &ReducerContext, username: String) -> Result<(), String> {
    ...
    let (
        ...
        let (spawn_pos, spawn_yaw) = select_spawn(ctx);
        (
            username,
            spawn_pos,
            spawn_yaw,
...
pub(crate) fn respawn_ready_players(ctx: &ReducerContext, server_tick: u64) {
    ...
    for mut health in health_rows {
        ...
        if let Some(mut transform) = ctx.db.player_transform().identity().find(identity) {
            ...
            transform.rotation_y = spawn_yaw;
...
/// Deterministic, count-driven spawn selection: the Nth currently-spawned player (by live
/// `player_transform` row count — one row per joined player; see `join_game` / `cleanup_player`)
/// lands on spawn `N % spawn_count`, cycling round-robin through every authored
/// `shared/arena.json` spawn as the population grows or wraps past it. Host-free and directly
/// unit-testable. `spawn_count == 0` degrades to index 0 — guards a malformed/empty arena.json;
/// the real `ARENA_SPAWNS` (via `tools/gen-arena`) always has >= 1 authored row.
pub(crate) fn select_spawn_index(occupied_count: u64, spawn_count: usize) -> usize {
    if spawn_count == 0 {
        return 0;
    }
    (occupied_count % spawn_count as u64) as usize
}

/// `(position, yaw)` for the spawn `select_spawn_index` picks against live `player_transform`
/// occupancy and `shared/arena.json`'s authored `ARENA_SPAWNS` (`collision.rs`, generated by
/// `tools/gen-arena`). Both `join_game` (new player) and `respawn_ready_players` (death) call
/// this so a player always lands on a real authored spawn row and facing — never the fixed
/// coordinate-origin stub this replaced.
fn select_spawn(ctx: &ReducerContext) -> (Vector3, f32) {
    let occupied = ctx.db.player_transform().count();
    let spawns = crate::collision::ARENA_SPAWNS;
    let index = select_spawn_index(occupied, spawns.len());
...
mod tests {
    ...
    #[test]
    fn select_spawn_index_degenerate_empty_spawns_is_index_zero() {
        assert_eq!(select_spawn_index(0, 0), 0);
        assert_eq!(select_spawn_index(7, 0), 0);
    }

    #[test]
    fn select_spawn_index_round_robins_deterministically() {
        // Four authored spawns (shared/arena.json): the Nth currently-spawned player lands on
        // N % 4, and a fifth player wraps back to spawn 0 rather than panicking out of range.
        assert_eq!(select_spawn_index(0, 4), 0);
        assert_eq!(select_spawn_index(1, 4), 1);
        assert_eq!(select_spawn_index(2, 4), 2);
        assert_eq!(select_spawn_index(3, 4), 3);
        assert_eq!(select_spawn_index(4, 4), 0);
        assert_eq!(select_spawn_index(5, 4), 1);
    }

    #[test]
    fn select_spawn_index_is_a_pure_deterministic_function_of_its_inputs() {
        // Same inputs, same output, every time — no hidden state, no randomness.
        for occupied in 0..20u64 {
            for spawn_count in 1..6usize {
                assert_eq!(
                    select_spawn_index(occupied, spawn_count),
                    select_spawn_index(occupied, spawn_count)
                );
            }
        }
    ...
    #[test]
    fn four_sequential_joins_land_on_all_four_authored_spawn_positions() {
        // Mirrors what `select_spawn` does against live `player_transform` occupancy counts of
        // 0, 1, 2, 3 (the first four players to join) without needing a `ReducerContext`: index
        // straight off `crate::collision::ARENA_SPAWNS`, the same table `select_spawn` reads.
        let spawns = crate::collision::ARENA_SPAWNS;
        assert_eq!(spawns.len(), 4, "shared/arena.json authors exactly 4 spawns");

        let mut distinct_positions = std::collections::HashSet::new();
        for occupied in 0..4u64 {
            let index = select_spawn_index(occupied, spawns.len());
            let spawn = &spawns[index];
            distinct_positions.insert((
                spawn.position[0].to_bits(),
                spawn.position[1].to_bits(),
                spawn.position[2].to_bits(),
            ));
        }
        assert_eq!(
            distinct_positions.len(),
            4,
            "four sequential joins must occupy four distinct spawn positions"
        );
    }

    #[test]
    fn arena_spawns_author_distinct_yaws_so_facing_is_not_hardcoded() {
        // `select_spawn` returns the spawn row's own yaw, not a fixed 0.0 — this asserts the
        // authored data actually gives that plumbing something to prove: distinct facings.
        let spawns = crate::collision::ARENA_SPAWNS;
        let distinct_yaws: std::collections::HashSet<_> =
            spawns.iter().map(|s| s.yaw.to_bits()).collect();
        assert_eq!(distinct_yaws.len(), spawns.len(), "authored spawns face distinct directions");
    }
}
