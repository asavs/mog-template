...
- **Avatar / equipment:** catalog + assembler live in `src/avatar/`. Classes are loadout **presets** (body + gear + grants), not mesh-pack identities. Do not add per-class animation folders or `preloadAllCharacter*` of every pack — preload the selected preset only. **Authority ids** (items/grants/presets) live in `shared/avatar-loadout.json` — edit that, then `npm run gen:loadout`. Presentation meshKeys/clips stay in `catalog.ts`. Doctrine: wiki `design/avatar-equipment.md` + `src/avatar/ART_DROP_IN.md`.
