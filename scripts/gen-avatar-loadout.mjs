...
function tsStringUnion(ids) {
  if (!ids.length) return 'never';
  return ids.map((id) => JSON.stringify(id)).join(' | ');
}

function tsConstStringArray(name, ids) {
...
${tsConstStringArray('BODY_IDS', bodyIds)}
export type BodyId = (typeof BODY_IDS)[number];

${tsConstStringArray('ITEM_IDS', itemIds)}
export type ItemId = (typeof ITEM_IDS)[number];

${tsConstStringArray('ABILITY_IDS', grantIds)}
/** Ability / grant id (same vocabulary as server grants). */
export type AbilityId = (typeof ABILITY_IDS)[number];

${tsConstStringArray('LOADOUT_PRESET_IDS', presetIds)}
